...
/*
function player_turn(sym, counter, index)	//function to take input from user when his/her turn comes
{
	var x;
	index[counter] = x+1;						//stores the value in an array for further reference
	t[x/3][x%3] = sym;						//stores the symbol of player in the 2-dimensional array
...
/*
function play_game ()		//function to start game
{
	var x = 'X', o = 'O';
	
	if (turn == 1)										//if the player wants to play first
	{
        if (counter == 1) {
            board.classList.add(X_CLASS)
        }
		while (counter<=9)					//looping through player and cpu turns
		{
            if (counter % 2 == 0)
			{
				cpu_turn(o, x, index);				//calling the function to get input position
				flag = 2 * check_win();					//checks if the cpu have won after this input
				if (flag == 2)
					break;							//if the cpu have won, then stop further looping (flag is 2)
                swapTurns()
                setBoardHoverClass()
			} else {
                while (waiting) {
                    setTimeout ("alert('click a cell')", 1250);
                }
            }
		}
	}
	else
	{
		...
		while (counter<=9)					//looping through player and cpu turns
		{
			if (counter%2 == 1)							//for odd turns, calls the cpu_turn function to get position
			{
				cpu_turn(x, o);
				flag = 2 * check_win();					//check if cpu have won after this input position
				if (flag == 2)
					break;							//if the cpu have won, then stop further looping (flag is 2)
                swapTurns()
                setBoardHoverClass()
                ...
                setTime ("alert('click a cell')", 1250)
            }
		}
	}											//after end of match prints the result accordingly and return to main
	if (flag == 0)
		winningMessageTextElement.innerHTML = 'Match Draw!!!'
	else if (flag == 1)
		winningMessageTextElement.innerHTML = 'Player wins!'
...
function initialize()											//main function for executing all the codes
{
    ...
    flag = 0;
    //winningMessageTextElement.innerHTML = flag;
    //clears the win detector for next match
    //play_game (index, turn, flag);					//calls the function to start game								//keep playing if the user gives input for that					//stops the game play and exit if the user gives input for that
}